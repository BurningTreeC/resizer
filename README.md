...
|----------|-------------|--------------|
| `<<actionValue>>` | The numeric value in the widget's unit | All actions |
| `<<actionValuePixels>>` | The value in pixels (always pixels regardless of unit) | All actions |
...
<$resizer
  ...
  dblClickActions="""
    ...
    <$action-log message="Panel width on double-click" value=<<actionValue>>/>
  """
/>
```

When using `dblClickActions`, the following variables are available:
- `<<actionValue>>` - The current value with unit
- `<<actionValuePixels>>` - The current value in pixels
- `<<actionDirection>>` - The resize direction
- `<<actionParentSize>>` - The parent container size in pixels
- `<<actionHandleSize>>` - The handle size in pixels
